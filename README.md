...
|---|---|---|---|
| Cristiano Ronaldo is the all-time top scorer in the Champions League with 140 goals. | Statistics/Records | 0.60 | ✅ |
| Diego Maradona has passed away at the age of 60. | News | 0.41 | ✅ |
| Promotion and relegation is the single best thing about European football. | Opinion | 0.37 | ✅ |
| Klopp post-match: These boys are unbelievable. | Opinion | 0.49 | ❌ (true: News) |
| Curacao has qualified for the 2026 World Cup, becoming the smallest nation ever. | Statistics/Records | 0.40 | ❌ (true: News) |

The first row is a good example of a reasonable prediction: the post is a pure standing record —
a named player, an all-time superlative, and a single number with no argument or recency — which
is exactly the surface signature the model learned for Statistics/Records, and it is the model's
most confident call in this set. The bigger thing to notice is that *every* confidence here sits
in the 0.37–0.60 range: the model is never very sure, because it is small and trained on only 140
examples. That modest confidence is itself useful information — a deployment could route anything
below about 0.6 to a human reviewer, which would catch both of the wrong predictions above.
...
contained unfilled `<label_1>` placeholders, and I did not take its output on faith: I checked the generated Statistics examples for factual plausibility, I confirmed
